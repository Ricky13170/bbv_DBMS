...
| **P2** | **Data Scanning**<br>`IRecordIterator`<br>`TableIterator`<br>`BTreeIterator` | **Iterator** | Query processing layers need to scan massive volumes of records without loading them all into memory at once. It also masks the physical page structures from the caller. | `iterator = table.create_iterator()`. The query layer safely calls `while iterator.has_next(): row = iterator.get_next()`, processing chunks iteratively. |
| **P2** | **Write-Ahead Logging**<br>`ILogRecord`<br>`InsertLog`<br>`UpdateLog`<br>`DeleteLog` | **Command** | To meet ACID Durability requirements, writes must be durably logged before hitting disk storage. If a crash occurs, these operations must be undone or redone. | Each `LogRecord` wraps the exact state change. During recovery, the Recovery Manager invokes `log_record.execute()` to Redo, or `log_record.undo()` to Rollback. |
| **P2** | **Concurrency Control**<br>`LockManager` | **Mediator** | When 100s of transactions rush to read/write the same `PageID`, they would deadlock or corrupt data if they interact directly. The Mediator coordinates this chaos. | Transactions don't talk directly to pages for locks. They signal `LockManager.acquire_lock(txn_id, page_id)`. The LockManager mediates queueing and grants. |
